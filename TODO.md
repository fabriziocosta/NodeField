...
- Add full tokenized-conditioning support at the graph-generator level so `ConditionalNodeFieldGraphGenerator`
  can work directly with structured condition memories, not only flat graph-level conditioning vectors plus
  node and edge count channels. This should cover:

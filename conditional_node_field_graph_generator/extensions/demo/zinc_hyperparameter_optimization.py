"""YAML-driven ZINC hyperparameter optimization helpers for thin notebooks."""
...
def _read_yaml_mapping(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("ZINC hyperparameter optimization config must contain a mapping.")
    return data
...
def _validate_zinc_hyperparameter_optimization_config(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    dataset["num_graphs"] = _require_positive_int(dataset, "num_graphs")
    dataset["min_size"] = _require_positive_int(dataset, "min_size")
    dataset["max_size"] = _require_positive_int(dataset, "max_size")
    if int(dataset["max_size"]) < int(dataset["min_size"]):
        raise ValueError("dataset.max_size must be >= dataset.min_size.")
    dataset["random_state"] = int(dataset.get("random_state", experiment["random_state"]))
...
def load_zinc_hyperparameter_optimization_config(path: str | Path) -> dict[str, Any]:
    """Load and validate the YAML config used by the thin ZINC optimization notebook."""
    config_path = Path(path).expanduser()
    if not config_path.is_file():
        raise FileNotFoundError(f"Config file does not exist: {config_path}")
    return _validate_zinc_hyperparameter_optimization_config(_read_yaml_mapping(config_path))
...
def run_zinc_hyperparameter_optimization(
    config: Mapping[str, Any],
    *,
    notebook_context: Mapping[str, Any],
) -> dict[str, Any]:
    """Train and score a random-search ZINC molecule-generation hyperparameter study."""
    config = _validate_zinc_hyperparameter_optimization_config(config)
    ...
    notebook_data_root = _context_path(
        notebook_context,
        "NOTEBOOK_DATA_ROOT",
    ...
    if output_config.get("run_dir"):
        ...
        artifact_domain = str(output_config.get("artifact_subdir") or "molecules")
    ...
    (artifact_root / "configs").mkdir(parents=True, exist_ok=True)
    (artifact_root / "metrics").mkdir(parents=True, exist_ok=True)
    (artifact_root / "samples").mkdir(parents=True, exist_ok=True)
...
def summarize_zinc_hyperparameter_results(result: Mapping[str, Any]) -> pd.DataFrame:
    """Return the sorted trial summary table from an optimization result bundle."""

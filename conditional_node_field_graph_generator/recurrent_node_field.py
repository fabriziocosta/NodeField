...
from .recurrent_interventions import apply_recurrent_intervention, normalize_interventions
...
def configure_recurrence(owner, latent_dimension, sigma, **options):
    ...
    options["recurrent_hidden_dimension"] = (
        options["recurrent_hidden_dimension"]
        if options["recurrent_hidden_dimension"] is not None
        else latent_dimension
    )
    options["recurrent_sigma_max"] = (
        options["recurrent_sigma_max"] if options["recurrent_sigma_max"] is not None else sigma
    )
    for key in (
        "recurrent_hidden_dimension",
        "recurrent_training_steps",
        "recurrent_detach_interval",
    ):
        value = options[key]
    ...
    if (
        mode == "recurrent_energy"
        and options["recurrent_sigma_min"] > options["recurrent_sigma_max"]
    ):
...
class RecurrentNodeFieldMixin:
    def _initialize_recurrent_modules(self, latent_dimension):
        ...
        self.recurrent_input_fusion = nn.Sequential(
            nn.Linear(2 * latent_dimension, latent_dimension),
            nn.GELU(),
            nn.LayerNorm(latent_dimension),
        )
        self.recurrent_state_head = nn.Sequential(
            nn.LayerNorm(latent_dimension),
            nn.Linear(latent_dimension, hidden),
            nn.GELU(),
            nn.Linear(hidden, hidden),
        )
        self.recurrent_state_norm = (
            nn.LayerNorm(hidden) if self.recurrent_state_normalization else nn.Identity()
        )

    def _initialize_recurrent_state(self, batch_size, node_count, device, dtype):
        return torch.zeros(
            batch_size, node_count, self.recurrent_hidden_dimension, device=device, dtype=dtype
        )

    def _encode_recurrent_with_condition(
        self, input_rows, hidden_state, global_condition_vector, node_mask=None
    ):
    ...
    def _update_recurrent_hidden(self, latent_tokens, hidden_state, node_mask=None):
        h = self.recurrent_state_norm(
            hidden_state + self.recurrent_update_scale * self.recurrent_state_head(latent_tokens)
        )
        return h if node_mask is None else h * node_mask.unsqueeze(-1)

    def _compute_recurrent_score_field(
        self, noisy_input, hidden_state, global_condition_vector, node_mask=None, *, create_graph
    ):
        with torch.enable_grad():
            ...
            z = self._encode_recurrent_with_condition(
                noisy_input, hidden_state, global_condition_vector, node_mask
            )
            ...
            score = -torch.autograd.grad(
                phi.sum(), noisy_input, create_graph=create_graph, retain_graph=True
            )[0]
    ...
    def _build_recurrent_sigma_schedule(self, steps, device, dtype):
        ...
        return torch.exp(
            torch.linspace(
                math.log(self.recurrent_sigma_max),
                math.log(self.recurrent_sigma_min),
                steps,
                device=device,
                dtype=dtype,
            )
    ...
    def _recurrent_node_field_loss(
        self,
        input_examples,
        global_condition,
        node_presence_mask=None,
        node_degree_targets=None,
        ...
        *,
        ...
        h = self._initialize_recurrent_state(
            *input_examples.shape[:2], input_examples.device, input_examples.dtype
        )
        schedule = self._build_recurrent_sigma_schedule(
            self.recurrent_training_steps, input_examples.device, input_examples.dtype
        )
        ...
        log_weights = input_examples.new_tensor(
            [(steps - 1 - k) * math.log(self.recurrent_loss_discount) for k in indices]
        )
        ...
        for k, sigma in enumerate(schedule):
            ...
            score, _, latent_noisy, h_next = self._compute_recurrent_score_field(
                noisy, h, global_condition, node_presence_mask, create_graph=create_graph
            )
            if self.recurrent_corruption_schedule == "none":
                score_loss = input_examples.new_zeros(())
            else:
                mask = self._build_node_field_score_mask(
                    input_examples,
                    node_presence_mask,
                    apply_sparse_supervision=apply_sparse_supervision,
                )
                score_loss = ((score + eps / sigma).square() * mask).sum() / mask.sum().clamp_min(
                    1.0
                )
            ...
            if k in indices:
                ...
                clean_latent = self._encode_recurrent_with_condition(
                    denoised, h, global_condition, node_presence_mask
                )
                ...
                if pair_targets is not None:
                    losses["total"], extra = self._additional_structural_losses(
                        losses["total"],
                        latent,
                        global_condition if count_condition is None else count_condition,
                        node_presence_mask,
                        *pair_targets,
                    )
                    losses.update(extra)
            ...
            h = h_next
            if not create_graph or (
                self.recurrent_detach_interval is not None
                and (k + 1) % self.recurrent_detach_interval == 0
            ):
                h = h.detach()
    ...
    def _recurrent_batch_step(self, batch, *, training):
        # Lightning may enter inference_mode during validation, which enable_grad alone cannot undo.
        if torch.is_inference_mode_enabled():
            with torch.inference_mode(False):
                return self._recurrent_batch_step(
                    tuple(
        ...
        with torch.enable_grad():
            losses, _ = self._recurrent_node_field_loss(
                x,
                self._apply_cfg_dropout(condition) if training else condition,
                mask,
                degree,
                labels,
                create_graph=training,
                apply_sparse_supervision=training,
                pair_targets=pairs,
    ...
    def recurrent_readout(self, x, h, condition, node_mask=None):
        ...
        with torch.no_grad():
            ...
            return dict(
                ...
                edge_probabilities=self._compute_edge_probability_matrices(z)
                if self.use_locality_supervision
                else None,
                edge_label_logits=self._compute_edge_label_logits(z)
    ...
    def generate_recurrent(
        ...
        desired_target=None,
        guidance_scale=1.0,
        global_condition_unconditional=None,
        ...
        *,
        ...
        rngs = [
            torch.Generator().manual_seed(item.seed if item.seed is not None else 0)
            for item in items
        ]
        self.eval()
        for name in (
        ...
        if trajectory is not None:
            trajectory.x.append(detached(x))
            trajectory.h.append(detached(h))
        ...
        for k in range(steps):
            ...
            h = h.detach()
            with torch.enable_grad():
                score, phi, _, hn = self._compute_recurrent_score_field(
                    x, h, global_condition, node_mask, create_graph=False
                )
                if use_cfg:
                    su, _, _, hun = self._compute_recurrent_score_field(
                        x,
                        hu.detach(),
            ...
            if trajectory is not None:
                ...
                metrics.update(prediction_deltas(readout, previous_readout))
                metrics["step"] = k
                trajectory.diagnostics.append(metrics)
                trajectory.evaluated_x.append(detached(x))
                trajectory.evaluated_h.append(detached(h))
                trajectory.x.append(detached(xn))
        ...
        self._last_recurrent_metadata = dict(
            field_evaluations=steps * (2 if use_cfg else 1),
            diagnostic_readouts=steps if return_trajectory else 0,
            final_readouts=int(use_heads_projection),
            diagnostic_seconds=diagnostic_seconds,
            runtime_seconds=time.perf_counter() - started,
            langevin_noise_scale=self.langevin_noise_scale,
            sampling_step_size=eta,
            initial_distribution="standard_normal",
            sigma_semantics="no sampling corruption schedule",
        )
        if trajectory is not None:

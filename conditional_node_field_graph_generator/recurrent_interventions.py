...
class RecurrentIntervention:
    ...
    def __post_init__(self):
        if self.kind not in {
            "none",
            "reset_hidden",
            "shuffle_hidden_nodes",
            "fresh_x_noise",
            "fresh_x_noise_every_step",
        }:
        ...
        if (
            self.kind not in {"none", "fresh_x_noise_every_step"}
            and self.step is None
            and not self.every_step
        ):
            raise ValueError("intervention requires step or every_step=True")

    def active(self, step):
        return self.kind != "none" and (
            self.every_step or self.kind == "fresh_x_noise_every_step" or self.step == step
        )
...
def normalize_interventions(intervention, steps):
    items = (
        ...
        if intervention is None
            ...
            [intervention]
            if isinstance(intervention, RecurrentIntervention)
            else list(intervention)
        )
    )
    for item in items:
...
def apply_recurrent_intervention(x, h, intervention, step, node_mask=None, *, generator=None):
    ...
    if generator is None:
        generator = torch.Generator().manual_seed(
            intervention.seed if intervention.seed is not None else 0
        )
    if intervention.kind == "reset_hidden":
        ...
        for b in range(h.shape[0]):
            valid = (
                ...
                else node_mask[b].nonzero().flatten()
            )
        ...
        x = (
            torch.randn(x.shape, generator=generator, dtype=x.dtype).to(x.device)
            * intervention.noise_scale
        )

"""Oracle-guided decode helpers for graph reconstruction."""
...
def decode_generated_nodes_with_oracle(
    ...
    predicted_node_labels_list = owner._resolve_predicted_node_labels(generated_nodes)
    predicted_edge_labels_list, predicted_edge_label_matrices = owner._resolve_predicted_edge_labels(
        generated_nodes,
    ...
    for graph_idx in range(len(predicted_edge_probability_matrices)):
        single_generated_nodes = owner._build_single_generated_node_batch(generated_nodes, graph_idx)
        ...
        current_edge_label_matrix = (
            ...
            else owner._edge_label_list_to_matrix(
        ...
        def plot_oracle_phase(
            ...
            *,
            ...
            owner._plot_decoder_diagnostics(
        ...
        ) -> tuple[nx.Graph, List[NodeSet], List[FrozenSet[Edge]], float, float, float, float]:
            candidate_graph = _assemble_graph_job(
                existence_mask,
                np.asarray(node_labels, dtype=object),
                np.asarray(owner._edge_label_matrix_to_list(single_adj_mtx, edge_label_matrix), dtype=object),
        ...
        for _iteration_idx in range(owner.max_oracle_iterations):
            ...
            current_edge_label_matrix = owner._edge_label_list_to_matrix(single_adj_mtx, edge_labels)
            (
                graph,
            ...
            new_node_forbidden: List[ForbiddenNodeLabelAssignment] = []
            ...
            new_edge_label_forbidden: List[ForbiddenEdgeLabelAssignment] = []
            ...
            structural_graph = _assemble_graph_job(
                existence_mask,
                current_node_labels,
                np.asarray(owner._edge_label_matrix_to_list(single_adj_mtx, current_edge_label_matrix), dtype=object),
                np.asarray(single_adj_mtx, dtype=float),
            )

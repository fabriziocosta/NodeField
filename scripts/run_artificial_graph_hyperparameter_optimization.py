#!/usr/bin/env python
"""Run the YAML-driven artificial-graph hyperparameter optimization workflow."""
...
from conditional_node_field_graph_generator.extensions.demo.artificial_hyperparameter_optimization import (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("notebooks/configs/artificial_graph_hyperparameter_optimization.yaml"),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = load_artificial_hyperparameter_optimization_config(config_path)
    result = run_artificial_hyperparameter_optimization(config, notebook_context=context)
    print(summarize_artificial_hyperparameter_results(result).to_string(index=False))
